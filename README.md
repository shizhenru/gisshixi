...
5. 进入「空间分析」，在「数据 A」和「数据 B」下拉框中选择两幅栅格影像。
6. 在拉帘画布中拖动中央竖线：左侧显示数据 A，右侧显示数据 B；滚轮缩放、拖拽平移，点击「复位视图」恢复全图。
7. 进入「工作台」，选择 Python、R（含「R 属性 GWR」真实实现）或「栅格 R / terra」后端，设置变量、核函数、带宽和距离度量。
8. 点击「运行分析」，在「结果与报告」查看指标并导出报告。
...
- 建议先在「预处理」页运行「栅格对齐」，客户端会优先使用 `desktop_client/.runtime/raster_preprocess/manifest.json` 中记录的对齐 GeoTIFF。
